              ...
              <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
                {activeBusinessData.images.map((image, index) => (
                  <button
                    key={image.id}
                    type="button"
                    onClick={() => setSelectedImage({ src: image.src, alt: `Imagen de ${activeBusinessData.name} para marketing en Durango ${index + 1}` })}
                  ...
                  >
                    <img
                      src={image.src}
                      alt={`Imagen de ${activeBusinessData.name} para marketing en Durango ${index + 1}`}
                      loading="lazy"
                      decoding="async"
